...
describe("<GoalInformation>", () => {
  describe("Functionality: ", () => {
    it("Display information", async () => {
      ...
      expect(wrapper.find(".goal__information").element.innerHTML).toContain(
        "4 monthly"
      );
      expect(wrapper.find(".goal__information").element.innerHTML).toContain(
        "$2000.00"
      ...
      expect(wrapper.find(".goal__information").element.innerHTML).toContain(
        momentFourMonthsFromNow.format("MMMM YYYY")
      );
      expect(
        wrapper.find(".goal__mothly-cost-value").element.innerHTML
      ).toContain("$500.00");
      ...
      expect(wrapper.find(".goal__information").element.innerHTML).toContain(
        momentFiveMonthsFromNow.format("MMMM YYYY")
      );
      expect(
        wrapper.find(".goal__mothly-cost-value").element.innerHTML
      ).toContain("$1000.00");
